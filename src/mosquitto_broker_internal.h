...
typedef struct Subscriptionlist {
	...
	struct mosquitto_client_msg *umsg;
	struct Subscriptionlist *next;

}Subscriptionlist;
typedef struct Min_mosq {
	int sub_count;
	struct Subscriptionlist *subs;
	struct mosquitto_msg_store *stored;
	struct Min_mosq* next;
	

}Min_mosq;
typedef struct Urgent_db {
	int message_count;
	struct Min_mosq *messages;
	char testbuf[100];
 ...
 * ============================================================ */
int handle__packet(struct mosquitto_db *db, struct mosquitto *context,Urgent_db *udb);
